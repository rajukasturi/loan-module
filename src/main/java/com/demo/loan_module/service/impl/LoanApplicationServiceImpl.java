...
public class LoanApplicationServiceImpl implements LoanApplicationService {
    ...
    private LoanApplication mapRequestToEntity(CreateLoanApplicationRequest request,
        ...
        application.setLoanType(request.getEffectiveLoanType());
        ...
        if (request.getGoldDetails() != null) {
            application.setGoldWeightInGrams(request.getGoldDetails().getGoldWeightInGrams());
            application.setGoldCarat(request.getGoldDetails().getGoldCarat());
            application.setGoldItemDescription(request.getGoldDetails().getGoldItemDescription());
        }
    ...
    private void determineAndSetStatus(LoanApplication application) {
        ...
        // Check if gold loan specific conditions are met
        boolean goldLoanEligible = loanEligibilityService.isGoldLoanEligible(application);

        if (!goldLoanEligible && application.isGoldLoan()) {
            application.setStatus(LoanStatus.REJECTED);
            logger.warn("Gold loan specific conditions not met - Application: {}", application.getId());
            return;
        }
    ...
    private LoanApplicationResponse mapEntityToResponse(LoanApplication application) {
        ...
        // Gold details (for gold loans)
        response.setGoldWeightInGrams(application.getGoldWeightInGrams());
        response.setGoldCarat(application.getGoldCarat());
        response.setGoldItemDescription(application.getGoldItemDescription());
